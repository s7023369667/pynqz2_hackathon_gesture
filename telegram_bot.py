...
class telegrambot():
    def __init__(self):
        token = '1950280286:AAHkmBj7qW9zZ6A_9D3pwYlD5M3XT_Fuqfs'
        self.bot = telegram.Bot(token=token)

    def send_message(self,message):
        mes = self.bot.send_message(chat_id=1963689302,text=message)
    ...
    def send_photo(self,res):
        ...
        photo = self.bot.send_photo(chat_id=1963689302,photo=imgur[res])
        self.del_message(photo.message_id)

    def del_message(self,del_id):
        time.sleep(2)
        self.bot.delete_message(chat_id=1963689302, message_id=del_id)
